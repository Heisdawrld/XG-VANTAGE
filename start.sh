...
# Step 1: Run database migrations (ensure all tables exist in Turso)
...
async function migrate() {
  ...
  for (const sql of tables) {
    try { await client.execute(sql); } catch (e) { console.log('Table exists:', sql.split(' ')[5]); }
  }
  for (const sql of indexes) {
    try { await client.execute(sql); } catch (e) { /* skip */ }
  }
  
  // Seed model_weights if empty
...
migrate().catch(e => { console.error('Migration error:', e); process.exit(0); });
" 2>&1 || echo "[Startup] Migration already applied"
